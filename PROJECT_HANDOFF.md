...
饱和 81×121 仍是高计算量任务，但 GUI 保持响应且可取消；共母线开绕组为理想协调调制口径；案例 A-B 仅直接比较共同坐标；铁耗系数需实测标定；发布包未签名。

## 下一步建议

在不改变结果口径的前提下，对饱和行求解实施相邻转矩/转速续算与局部搜索，然后增加 Ψd/Ψq Map 和开绕组共模/环流模型。
...
饱和 11×11：后台 6.23 s，外特性 4.92 s，Map 1.30 s，平均内部点 10.7 ms，最慢折算点 11.8 ms，失败 0，GUI 刷新 0.51 s。11×121：37.82 s，失败 0；81×121 估算约 5–6 min。
